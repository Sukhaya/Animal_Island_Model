...
public class Boa extends Predator {
    ...
    protected int getProbabilityOfEating(Animal species) {
        if (species instanceof Fox) return SimulationSettings.EatingChance.BOA_FOX;
        if (species instanceof Rabbit) return SimulationSettings.EatingChance.BOA_RABBIT;
        if (species instanceof Mouse) return SimulationSettings.EatingChance.BOA_MOUSE;
        if (species instanceof Duck) return SimulationSettings.EatingChance.BOA_DUCK;
        return 0;
    }
